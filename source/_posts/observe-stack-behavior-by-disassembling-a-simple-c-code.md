...
- 第17、18行將`a(1,2)`中的兩個參數分別存到`esi`和`edi`寄存器中，這個地方需要說明的是
  - x86-32中會使用push將實參存入stack內，如下圖（原`main.c`反編譯成 32-bit ASM 代碼）：
  ...
  - 而在 x86-64 中，如果參數在6個以內，GCC 就可以利用寄存器來存儲參數值；超過6個的參數，還是通過上述操作實現
...
- 然後是爲了return 0把0給`eax`

- 因爲是main函數，所以最後是leave

- leave: Releases the local stack storage created by the previous ENTER instruction.
